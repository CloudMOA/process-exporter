...
PREFIX                  ?= $(shell pwd)
BIN_DIR                 ?= $(shell pwd)
DOCKER_IMAGE_NAME       ?= datasaker/process-exporter
...
SMOKE_TEST = --config.path packaging/conf/all.yaml --once-to-stdout-delay 1s |grep -q 'process_group_memory_bytes{groupname="process-exporte",memtype="virtual"}'
